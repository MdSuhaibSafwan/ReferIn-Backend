...
exports.stripeWebhook = (request, response) => {
  ...
  if (event.type == "checkout.session.completed") {
    ...
    if (frontendUid == null) {
      ...
      StripeSession.findSessionByUid(frontendUid).then((resp) => {
        if (resp.data.length > 0) {
          var sessionObjId = resp.data[0].id;
          StripeSession.updateSession(sessionObjId, { has_paid: true }).then(
            (resp) => {
              if (resp.data.length > 0) {
                response
                  .status(200)
                  .send({ message: "has been paid successfully" });
              } else {
                return response
                  .status(404)
                  .send({ message: "no session found" });
              }
            }
          );
        } else {
